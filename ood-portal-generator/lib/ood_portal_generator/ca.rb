module OodPortalGenerator
  ...
  class CA
    ...
    def create_ca(name)
      ...
      open @key_path, 'w', 0400 do |io|
        io.write ca_key.private_to_pem('aes-256-cbc', @key_phrase), perm: 0400
      end
    ...
    # @return [Boolean] whether or not the cert exists
    def exists?
      File.file?(cert_path) and File.file?(key_path)
    end
  end
  ...
  class UserCA < CA
    def initialize(opts = {})
      ...
      @key_phrase = opts.fetch(:ondemand_user_ca_passphrase, '')
  ...
  class OndemandCA < CA
    ...
    def initialize(opts = {})
      ...
      @key_phrase = opts.fetch(:ondemand_ca_passphrase, '')
