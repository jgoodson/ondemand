  ...
  class PunConfigGenerator < Generator
    ...
    add_hook :block_user_with_disabled_shell do
      if NginxStage.disabled_shell.kind_of?(Array)
        raise InvalidUser, "user has disabled shell: #{user}" if (NginxStage.disabled_shell.include?(user.shell))
      else
    ...
    add_hook :create_user_certificate do
      begin
        cert = UserCertificate.new(user)
        cert.generate unless cert.valid?
      rescue => e
        $stderr.puts "Failed to create user certificate: #{cert.cert_path}"
        $stderr.puts e.message
        $stderr.puts e.backtrace
